...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global['@dashi0/html2canvas'] = factory());
    ...
    ***************************************************************************** */
    ...
    var getColorFunctionValue = function (token) {
        if (token.type === 17 /* NUMBER_TOKEN */) {
            return token.number;
        }
        if (token.type === 16 /* PERCENTAGE_TOKEN */) {
    ...
    var colorFunction = function (_context, args) {
        var tokens = args.filter(nonFunctionArgSeparator);
        if (tokens.length < 4) {
        ...
        if (colorSpace.type !== 20 /* IDENT_TOKEN */) {
            return 0;
        }
        var colorSpaceName = colorSpace.value.toLowerCase();
        // For now, we'll support srgb, srgb-linear, display-p3, a98-rgb, prophoto-rgb, rec2020
        // All will be converted to sRGB for display
        var _a = tokens.slice(1, 4).map(function (token) { return getColorFunctionValue(token); }), c1 = _a[0], c2 = _a[1], c3 = _a[2];
        // Handle alpha channel if present (after slash)
        var alpha = 1;
        if (tokens.length >= 6) {
            // Look for slash separator and alpha value
            var slashIndex = tokens.findIndex(function (token, index) {
                return index >= 4 && token.type === 6 /* DELIM_TOKEN */ && token.value === '/';
            });
            if (slashIndex !== -1 && slashIndex + 1 < tokens.length) {
                var alphaToken = tokens[slashIndex + 1];
